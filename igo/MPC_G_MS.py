...
def _active_dim_mask(dim, D_m):
    return jnp.arange(dim) < D_m

def _masked_precision(S, D_m):
    """Keep valid dimensions from S and set padded dimensions to identity."""
    mask = _active_dim_mask(S.shape[0], D_m)
    active = mask[:, None] & mask[None, :]
    invalid_diag = jnp.diag((~mask).astype(S.dtype))
    return jnp.where(active, S, 0.0) + invalid_diag
...
def _gaussian_log_pdf_l_masked(xi, mu, S, D_m):
    """带维度掩码的高斯对数概率密度计算"""
    diff = (xi - mu)
    mask = _active_dim_mask(xi.shape[0], D_m)
    diff = diff * mask
    S_eff = _masked_precision(S, D_m)
    
    mahalanobis_sq = jnp.dot(diff, jnp.dot(S_eff, diff))
    sign, logdet_S = jnp.linalg.slogdet(S_eff)
...
def _update_block_component_core(
    ...
    dim = samples.shape[-1]
    mask = _active_dim_mask(dim, D_m)
    active = mask[:, None] & mask[None, :]
    S_k_eff = _masked_precision(S_k, D_m)
    diff = (samples - mu_k) * mask

    def s_grad_fn(d):
        grad = S_k_eff @ jnp.outer(d, d) @ S_k_eff - S_k_eff
        return jnp.where(active, grad, 0.0)
    
    sum_S_grad = jnp.sum((elite_weights * a_i)[:, None, None] * vmap(s_grad_fn)(diff), axis=0)
    S_new = _masked_precision(_safe_spd_projection(S_k_eff - delta_t * sum_S_grad), D_m)

    S_diff = (S_k_eff @ diff.T).T
